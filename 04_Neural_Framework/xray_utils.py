...
def show_detailed_math(title, components, result, operation="*", label=None):
    ...
    if len(parts) == 1:
        ...
        if operation == "*": 
            expr = MatMul(parts[0], parts[1], evaluate=False)
        elif operation == ".*": 
            from sympy.matrices.expressions.hadamard import HadamardProduct
            # Use HadamardProduct for clean element-wise visualization
            expr = HadamardProduct(parts[0], parts[1], evaluate=False)
        elif operation == "-": 
            expr = MatAdd(parts[0], -parts[1], evaluate=False)
        elif operation == "+": 
            expr = MatAdd(parts[0], parts[1], evaluate=False)
        else: 
            expr = res_m
...
def visualize_network(X, W1, W2, h_act, preds, sample_idx=0):
    ...
    # Layer 2: Hidden (4)
    # Layer 3: Output (1)
    # Dynamically determine layer sizes from the shapes of X, W1, and W2
...
def visualize_backprop(X, W1, W2, d_hidden, d_out, dW1, dW2, sample_idx=0):
    """
    Draws a clear visualization of the BACKWARD pass with solid nodes and clear labels.
    """
    plt.style.use('dark_background')
    fig, ax = plt.subplots(figsize=(12, 8))
    
    input_size = W1.shape[0]; hidden_size = W1.shape[1]; output_size = W2.shape[1]
    layer_sizes = [input_size, hidden_size, output_size]
    v_spacing = 1.5; h_spacing = 3.0
    node_coords = []
    
    # 1. Create Coordinates and Draw Nodes
    for i, size in enumerate(layer_sizes):
        layer_coords = []
        x = i * h_spacing
        top = (size - 1) * v_spacing / 2
        for j in range(size):
            ...
            # Use Solid Neon Colors
            color = '#ff007f' if i == 2 else ('#39ff14' if i == 1 else '#00d4ff')
            circle = plt.Circle((x, y), 0.2, color=color, alpha=1.0, zorder=5)
            ax.add_artist(circle)
            
            # Label Error Signals (δ)
            if i == 2: val = d_out[sample_idx, j]
            elif i == 1: val = d_hidden[sample_idx, j]
            else: val = 0 
            
            if i > 0:
                ax.text(x, y + 0.35, f"δ={val:.4f}", color=color, ha='center', fontweight='bold', fontsize=12)
                
        node_coords.append(layer_coords)

    # 2. Draw Backprop Arrows and Gradient Labels
    gradients = [dW1, dW2]
    for i in range(len(layer_sizes) - 1, 0, -1):
        for j in range(layer_sizes[i]): # Target (Right)
            for k in range(layer_sizes[i-1]): # Source (Left)
                end = node_coords[i-1][k]
                start = node_coords[i][j]
                grad_val = gradients[i-1][k, j]
                
                # Draw Arrow
                ax.annotate("", xy=end, xytext=start,
                            arrowprops=dict(arrowstyle="->", color='white', alpha=0.5, lw=2))
                
                # Label the Update (ΔW)
                mid_x = (start[0] + end[0]) / 2
                mid_y = (start[1] + end[1]) / 2
                ax.text(mid_x, mid_y + 0.1, f"ΔW={grad_val:.4f}", color='#ffff00', 
                        fontsize=10, ha='center', fontweight='bold', bbox=dict(facecolor='black', alpha=0.5, pad=1))

    # 3. Final Styling
    ax.set_xlim(-1, 7)
    ax.set_ylim(-3, 3)
    ax.set_axis_off()
    plt.title("BACKPROPAGATION: Signal Flow & Updates", fontsize=18, fontweight='bold', color='#ff00ff', pad=25)
    
    save_path = '/Users/indra/Desktop/DEEP_LEARNING/03_Neural_Networks_Scratch/backprop_state.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"\n🧠 BACKPROP VISUALIZATION REGENERATED: {save_path}")
    plt.close()
